...
class Box(object):
    ...
    def double(self):
        ...
        output = self.append_string(self.contents)
        return output
    ...
    def shrink(self, new_volume):
        ...
        #Then return the cut off part of the string
            for k in range(self.volume, len(self.contents)):
                newnewstring = newnewstring + self.contents[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        sone = self.double()
        stwo = self.shrink(new_volume)
        return (len(sone) + len(stwo))
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.storecontents = self.storecontents + [self.contents]
